...
export default function CASParserPage() {
  ...
  const handleExport = () => {
    ...
    // Sheet 3: Equities Direct
    const directEquities = result.equities.filter((_, i) => (equityTypes[`${i}`] || _.holdingType) === "DIRECT");
    if (directEquities.length > 0) {
      const eqHeaders = ["ISIN", "Stock Symbol", "Company Name", "Face Value", "Quantity", "Market Price (₹)", "Value (₹)", "Depository", "DP Name"];
      const eqRows = directEquities.map(e => [e.isin, e.stockSymbol || "", e.companyName, e.faceValue || "", e.quantity, e.marketPrice || "", e.value, e.accountType, e.dpName]);
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([eqHeaders, ...eqRows]), "Equities - Direct");
    }

    // Sheet 4: Equities PMS
    const pmsEquities = result.equities.filter((_, i) => (equityTypes[`${i}`] || _.holdingType) === "PMS");
    if (pmsEquities.length > 0) {
      const eqHeaders = ["ISIN", "Stock Symbol", "Company Name", "Face Value", "Quantity", "Market Price (₹)", "Value (₹)", "Depository", "DP/PMS Name"];
      const eqRows = pmsEquities.map(e => [e.isin, e.stockSymbol || "", e.companyName, e.faceValue || "", e.quantity, e.marketPrice || "", e.value, e.accountType, e.dpName]);
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([eqHeaders, ...eqRows]), "Equities - PMS");
    }

    // Sheet 5: Bonds
    if (result.bonds.length > 0) {
      const bHeaders = ["ISIN", "Company Name", "Coupon Rate", "Frequency", "Maturity Date", "No. of Bonds", "Face Value/Bond (₹)", "Market Price/Bond (₹)", "Value (₹)"];
      const bRows = result.bonds.map(b => [b.isin, b.companyName, b.couponRate || "", b.frequency || "", b.maturityDate || "", b.noOfBonds || "", b.faceValuePerBond || "", b.marketPricePerBond || "", b.value]);
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([bHeaders, ...bRows]), "Bonds");
    }

    // Sheet 6: AIF
    if (result.aif.length > 0) {
      const aifHeaders = ["ISIN", "Fund Name", "Units", "NAV (₹)", "Value (₹)", "DP/Manager"];
      const aifRows = result.aif.map(a => [a.isin, a.description, a.units, a.nav, a.value, a.dpName]);
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([aifHeaders, ...aifRows]), "AIF");
    }

    // Sheet 7: Portfolio Trend
    if (result.portfolioTrend.length > 0) {
      const tHeaders = ["Month", "Portfolio Value (₹)", "Change (₹)", "Change (%)"];
      const tRows = result.portfolioTrend.map(t => [t.month, t.value, t.change || "", t.changePct || ""]);
      XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([tHeaders, ...tRows]), "Portfolio Trend");
    }

    const investorName = result.investor.name?.replace(/\s+/g, "_") || "CAS";
    XLSX.writeFile(wb, `${investorName}_CAS_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
      ...
      <div className="flex items-center justify-between">
        ...
        {result && (
          <button onClick={handleExport}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-500 text-white text-sm font-medium hover:bg-brand-600 transition-all">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="px-3 py-1 rounded-full text-xs font-semibold bg-brand-500/10 text-brand-400">
                {result.investor.casType}
              </div>
              <span className="text-sm text-muted-foreground">
                {result.investor.name} {result.investor.pan && `· ${result.investor.pan}`}
              </span>
              {result.investor.totalPortfolioValue && (
                <span className="text-sm font-semibold text-foreground">
                  · {formatINR(result.investor.totalPortfolioValue)}
                </span>
              )}
            </div>
                    ...
                    <tbody>
                      {result.equities.map((eq, i) => (
                        <tr key={i} className="border-b border-border/50 hover:bg-accent/20">
                          ...
                          <td className="py-2.5 px-4">
                            <button
                              onClick={() => setEquityTypes(prev => ({ ...prev, [`${i}`]: prev[`${i}`] === "PMS" ? "DIRECT" : "PMS" }))}
                              className={cn("flex items-center gap-1 px-2 py-0.5 rounded-full text-2xs font-semibold transition-all cursor-pointer border",
                                (equityTypes[`${i}`] || eq.holdingType) === "PMS"
                                  ? "bg-amber-500/10 text-amber-400 border-amber-500/20"
                                  : "bg-blue-500/10 text-blue-400 border-blue-500/20"
                              )}>
                              <Tag className="h-2.5 w-2.5" />
                              {equityTypes[`${i}`] || eq.holdingType}
                            </button>
                          </td>
                        </tr>
                      ))}
